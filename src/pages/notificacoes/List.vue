...
export default defineComponent({
  ...
  setup () {
    ...
    const handleEdit = (item) => {
      router.push({ name: 'form-notificacao', params: { id: item.id } })
    }
    ...
    const handleRemove = async (item) => {
       try {
        $q.dialog({
          title: 'Confirmação',
          message: `Deseja realmente remover a notificação: "${item.titulo}"?`,
          cancel: true,
          persistent: true
        }).onOk(async () => {
