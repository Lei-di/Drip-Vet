...
export default defineComponent({
  ...
  setup () {
    ...
    const handleEdit = (item) => {
      router.push({ name: 'form-vacinacao', params: { id: item.id } })
    }
    ...
    const handleRemove = async (item) => {
      try {
        $q.dialog({
          title: 'Confirmação',
          message: `Deseja realmente remover o registro de vacina para ${item.pet_nome}?`,
          cancel: true,
          persistent: true
        }).onOk(async () => {
